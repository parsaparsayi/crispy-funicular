...
def tts():
    try:
        ...
        return jsonify(ok=False, error=f"TTS server error: {e}"), 500
...
def animate():
    try:
        ...
        audio_url = (data.get("audio_url") or "").strip()  # "" forces local fallback in grok
        ...
        return jsonify(ok=False, error=f"Animate server error: {e}"), 500
...
def full():
    try:
        ...
        if upload:
            ...
            audio_url = ""  # force local fallback
        ...
        return jsonify(ok=False, error=f"Full server error: {e}"), 500
